...
$(document).ready(function () {
	...
	var currentForm = 0;
	var forms = $('.right-col .step');
	$('.right-col .step form').submit(function(e){
		e.preventDefault();
		$('.step.active').removeClass('active');
		currentForm++;
		console.log(forms.eq(currentForm));
		forms.eq(currentForm).addClass('active');
		if (deviceWidth < 992) {
			var isSafari = /safari/.test(navigator.userAgent.toLowerCase());
			if (isSafari) {
		 		$('body').animate({ scrollTop: $('.step.active').offset().top - 110 }, 500);
			} 
		    else {
		     	$('html').animate({ scrollTop: $('.step.active').offset().top - 110 }, 500);
		    }
		}
	});
